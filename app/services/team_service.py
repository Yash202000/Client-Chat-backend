...
def get_teams(db: Session, company_id: int, skip: int = 0, limit: int = 100):
    return db.query(models_team.Team).options(selectinload(models_team.Team.members).joinedload(models_team_membership.TeamMembership.user)).filter(models_team.Team.company_id == company_id).offset(skip).limit(limit).all()
